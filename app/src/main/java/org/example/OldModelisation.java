...
public class OldModelisation {
    ...
    */
    ...
    static int nbCont = 20, nbStop = 4, nbBaies = 2, nbBlocs = 2, nbPiles = 2, hMax = 4;
    static int maxPos = (nbBaies - 1) * 1000 + (nbBlocs - 1) * 100 + (nbPiles - 1) * 10 + hMax + 1;
    static int nbPos = nbBaies * nbBlocs * nbPiles * hMax;
    static int[][] plannification;

    public static void main(String[] args) throws IOException {
        Model model = new Model("Chargement navire porte-container");

//        Generator generator = new Generator();
//        plannification = generator.t;

        IntVar[][] position = model.intVarMatrix("position",nbCont,nbStop, -1, maxPos, false);
//        IntVar[][] container = model.intVarMatrix("container", nbPos, nbStop, -1, nbCont - 1, false);
        SetVar[][] blocked = model.setVarMatrix("blocked", nbCont, nbStop, new int[0], allPos());
        ...
        for (int i=0 ; i<nbStop ; i++){
            ...
            for (int c=0 ; c<nbCont ; c++){
                ...
                // condition de pile
                model.ifThen(
                        model.and(model.arithm(position[c][i].mod(10).intVar(), "!=", hMax/2),
                                model.arithm(position[c][i].mod(10).intVar(), "!=", 0),
                                model.arithm(position[c][i], "!=", -1)),
                        model.count(position[c][i].sub(1).intVar(),
                ...
                model.ifThen(
                        model.arithm(position[c][i].mod(10).intVar(), ">", 0),
                        model.member(position[c][i].sub(1).intVar(), blocked[c][i])
                );

                // propagation blocked
                model.ifThen(
                        model.and(
                                model.member(position[c][i], blocked[c][i]),
                                model.arithm(position[c][i].mod(10).intVar(), "=", hMax/2)
                        ),
                        model.member(position[c][i].sub(1).sub(position[c][i].div(10).mod(100).mul(10)).intVar(),
                                blocked[c][i])
                );
                model.ifThen(
                        model.and(
                                model.member(position[c][i], blocked[c][i]),
                                model.arithm(position[c][i].mod(10).intVar(), "=", hMax/2 - 1),
                                model.arithm(position[c][i].div(10).mod(100).intVar(), "<", nbPiles - 1)
                        ),
                        model.member(position[c][i].add(10).intVar(), blocked[c][i])
                );

                // si un container en bloque un autre, il faut le déplacer pour bouger le bloqué
                for (int d = 0; d < nbCont; d++) {
                    model.ifThen(
                            model.and(
                                    model.member(position[d][i], blocked[c][i]),
                                    model.arithm(move[d][i], "!=", 0)
                            ),
                ...
                if(i < nbStop - 1) {
                    // move=1 ssi le container est chargé ou déchargé
                    model.ifOnlyIf(
                            model.and(
                    ...
                    // move=0 ssi la position du container n'a pas changé
                    ...
                    //move=2 ssi le container a changé de position
                    model.ifOnlyIf(
                            model.and(
                                    model.arithm(position[c][i], "!=", position[c][i + 1]),
                                    model.and(
        ...
        System.out.println(Arrays.deepToString(plannification));
        writeSolution(generatePosToCont(position, solution), translatePos());
    }
    private static int load(int c) {return plannification[c][0];}
    private static int dest(int c) {return plannification[c][1];}
    ...
    private static int[] allPos() {
        int[] t = new int[nbBaies * nbBlocs * nbPiles * (hMax - 1)];
        int j = 0;
        for (int nba = 0; nba < nbBaies; nba++) {
